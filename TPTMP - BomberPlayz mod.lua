...
local function disconnected(reason)
	...
	if reason then
		chatwindow:addline("disconnected: "..reason,255,50,50)
		lastchan = ''
	else
		chatwindow:addline("You have been disconnected! ",255,50,50)
	end
	con.connected = false
	con.members = {}
	end
...
local function conSend(cmd,msg,endNull)
	if not con.connected then return false,"HEY! You have to connect first!" end
	msg = msg or ""
...
function reconnect(nicka)
	local lasted = areconnect
	areconnect = false
	disconnected()
	if nicka == nil then
	connectToServer(lastip,lastport,username)
	else
	connectToServer(lastip,lastport,nicka)
	end
	if not(lastchan=='') then 
		joinChannel(lastchan) 
	end
	areconnect = lasted
end

local function getname (val)
    for index, value in ipairs(con.members) do
...
new=function(x,y,w,h)
	...
	rejoin = function(self, msg, args)
		reconnect()
	end,
	cheat = function(self, msg, args)
		if args[1] == "autoreconnect" then
			if areconnect == false then
			areconnect = true
			self:addline("Turned on AutoReconnect")
			else
			areconnect = false
			self:addline("Turned off AutoReconnect")
			end
		end
	end,
	autokick = function(self, msg, args)
		if getname(args[1]) then
			if args[2] == nil then  
				self:addline("Needed: Duration")
			else
				for i=1, args[2] do
					
					if getname(args[1]) then
						conSend(21, args[1].."\0".."You have been AutoKicked!",true)
					end
				end
			end
		else
		self:addline("User not found!")
		end
	end,
	lobby = function(self, msg, args)
		joinChannel("null")
		self:addline("Teleported to lobby!")
	end,
	dosync = function(self, msg, args)
		if args[1] == "display" then
			if nodisplay == 0 then
				nodisplay = 1
				self:addline("Turned on no display sync mode")
			else
				nodisplay = 0
				self:addline("Turned off no display sync mode")
			end
		end
	end,
	nick = function(self, msg, args)
		if not args[1] then
			self:addline("Required name")
		else
			reconnect(args[1])
		end
	end
...
local function connectThink()
	...
	while 1 do
		...
		if s then
			...
			if r ~= "timeout" then _print("Whoops! It look like you timed out :(",255,20,20) disconnected() end
			break
		end
	end
...
local keyFuncs = {
	...
	[48] = function() if nodisplay == 0 then conSend(48,"\10") end end,
	...
	[50] = function() if nodisplay == 0 then conSend(48,"\1") end end,
	[51] = function() if nodisplay == 0 then conSend(48,"\2") end end,
	[52] = function() if nodisplay == 0 then conSend(48,"\3") end end,
	[53] = function() if nodisplay == 0 then conSend(48,"\4") end end,
	[54] = function() if nodisplay == 0 then conSend(48,"\5") end end,
	[55] = function() if nodisplay == 0 then conSend(48,"\6") end end,
	[56] = function() if nodisplay == 0 then conSend(48,"\7") end end,
	[57] = function() if nodisplay == 0 then conSend(48,"\8") end end,
